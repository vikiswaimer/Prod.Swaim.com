...
Канон для мышления и правок: [`docs/methodology.md`](methodology.md) — Inspired (Cagan), Lean Startup (Ries), Mom Test (Fitzpatrick), Sprint (Knapp), MVP как ценный рабочий срез (Olsen + владелец). Навыки, скиллы и «сила» (команда, инструменты, ИИ) наращивать **соразмерно** результатам, которые окупаются денежной прибылью проекта — не впрок и не ради ощущения роста.
...
**Paperclip / Product Lead (2026-08-15):** sync — [`docs/reports/paperclip-team-sync.md`](reports/paperclip-team-sync.md); W1 brief — [`docs/reports/w1-mom-test-brief.md`](reports/w1-mom-test-brief.md); **SWA-9** decision — [`docs/reports/swa9-next-after-risk-map.md`](reports/swa9-next-after-risk-map.md); **SWA-10** productivity review — [`docs/reports/swa10-productivity-review-swa9.md`](reports/swa10-productivity-review-swa9.md) (resolve: productive + stop churn); risk-map — [`docs/risk-map-hypotheses.md`](risk-map-hypotheses.md). **WIP=1:** только W1 Mom Test.
...
8. **Paperclip-задачи флоту:** формулировать как цикл — гипотеза → тонкий срез → сигнал → done. Коротко (цель ~2000 байт UTF-8; лимит Cursor Cloud `envVars` = 4096 байт/значение). Длинное эссе ломает запуск `cursor_cloud`.
...
- Cloud Agent из Paperclip клонирует GitHub: `https://github.com/vikiswaimer/Prod.Swaim.com.git`. Origin для флота не используем.
...
- VM / сайт / серверный агент (Environments): [`swaim-environments-vm-trigger.md`](swaim-environments-vm-trigger.md) — триггер, пока не актуально.
