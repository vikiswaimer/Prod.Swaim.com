...
1. Тон по умолчанию после пакета C: вежливое **«вы»**, язык **RU** (EN только как осознанный термин).
2. Плейсхолдеры ответов: короткий пример в отдельном файле `examples/`, не в основном шаблоне.
3. При переименовании файлов/папок — обновить `00-index.md`, `INDEX.md` и входящие markdown-ссылки; прогнать проверку «битых» ссылок.
4. CSV = схема/данные баз; менять осмысленно и описывать перенос в Notion (свойства, формулы, views).
5. Не удалять массово «Библиотека знаний» без отдельного issue — там сотни файлов.
6. Любая правка смысла продукта сверяется с [`methodology.md`](methodology.md) (Mom Test, гипотезы, MVP как ценный срез и т.д.).
