...
Черновики тел: [`docs/issue-drafts/`](issue-drafts/README.md).
...
Частичный срез сигнала (каналы + Отзывы + Интервью в Other): issue **#19**, отчёт `docs/reports/feedback-channels.md`. Полный пакет E (Клиенты / persona) — ещё впереди.
...
1. **Один чат = один issue = одна цель** (по умолчанию). Не дробить чат на несколько issues/PR и не смешивать пакеты — пока владелец явно не попросит отдельные issue из этого чата.
...
- По умолчанию агент ведёт **один issue на чат** (создаёт/привязывает автоматически под задачу чата). Отдельные issues из одного чата — только по явной просьбе владельца.
- Владелец может сам создать issue заранее; иначе агент оформляет тикет под scope чата.
- Агент правит Git; по договорённости — `main` или ветка `cursor/<name>-ffef` + PR (**один** draft PR на чат/issue, если не попросили иначе).
