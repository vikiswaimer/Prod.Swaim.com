...
6. Положить секрет туда, где его видит агент:
   - **Cursor Cloud / environment secrets:** переменная `NOTION_TOKEN`
   - опционально `NOTION_ROOT_PAGE_ID` — id корневой страницы Small Business Space (из URL: `notion.so/...?p=...` или хвост URL страницы).
